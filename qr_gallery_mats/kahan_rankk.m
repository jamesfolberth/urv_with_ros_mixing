function A = kahan_rankk(N, k, theta, tau)
% KAHAN  Rank k Kahan matrix with Demmel/Grigori perturbation
%
...
if nargin < 3, theta = acos(0.6); end
if nargin < 4, tau = 1e-7; end
...
svec = s.^(0:n-1).';
%svec(k+1:end) = eps(svec(k));
svec(k+1:end) = eps(svec(k+1:end));
A = bsxfun(@times, bsxfun(@times,A,svec), (1-tau).^(0:n-1)); % probably less roundoff
